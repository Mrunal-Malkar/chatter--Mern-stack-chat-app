...
const Login = () => {
  ...
  const handleSubmit=async(e)=>{
    ...
    return window.location.href=`/login?error=${res.error || "Invalid login"}`;
  ...
  return (
    <div className='w-full min-h-[100vh] flex justify-center algin-middle'>
      ...
      <div className='w-[300px] md:w-[400px] flex flex-col justify-center items-center'>
        ...
        <form onSubmit={handleSubmit} method='POST' className='flex flex-col gap-y-4 p-4 bg-[#2D232E] w-full text-xl rounded-xl border-2 border-gray-900'>

          <div className='flex flex-col m-1 w-full'>
            <label htmlFor="email" className='text-gray-300'>Email:</label>
            <input type="text" onChange={(e)=>setEmail(e.target.value)} className='text-xl md:text-2xl rounded-xl p-1 outline-none' name='email' />
          </div>

          <div className='flex flex-col m-1 w-full'>
            <label htmlFor="password" className='text-gray-300'>Password</label>
            <input type="text" onChange={(e)=>setPassword(e.target.value)}  className='text-xl md:text-2xl rounded-xl p-1 outline-none' name='password' />
          </div>

          <div className='flex w-full justify-center items-center '>
            <input type='submit' className='p-3 bg-[#201A20] text-gray-200 rounded-2xl' value="Submit"/>
          </div>
